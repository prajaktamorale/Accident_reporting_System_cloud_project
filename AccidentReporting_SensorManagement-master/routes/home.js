...
function providerDashboard(req,res) {
	res.render("providerDashboard",{firstname:req.session.adminfirstname, lastname:req.session.adminlastname});
}
...
function getLoggedinUser(req,res) {
	var json_responses={};
	json_responses.email = req.session.email;
	res.send(json_responses); 
}
...
function addUser(req, res){
	...
	mongo.connect(mongoURL, function() {
		...
		coll.insert({firstname : firstname, lastname : lastname, password : password, cpswd : cpswd, email : email, phone : phone, flag:0}, function(err,user){
			if (user) {
...
function userLogin(req,res){
	...
	console.log("pwd: "+req.param("login"));
